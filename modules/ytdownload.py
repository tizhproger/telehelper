...
async def riper(self, m, type):
    ...
    m = await utils.answer(m, self.strings["preparing"])
    if type == "audio":
        opts = {
            "format": "bestaudio",
            "addmetadata": True,
    ...
    try:
        ...
        return await utils.answer(m, self.strings["err"].format(str(DE)))
    except ContentTooShortError:
        return await utils.answer(m, self.strings["content_too_short"])
    except GeoRestrictedError:
        return await utils.answer(m, self.strings["geoban"])
    except MaxDownloadsReached:
        return await utils.answer(m, self.strings["maxdlserr"])
    except PostProcessingError:
        return await utils.answer(m, self.strings["pperr"])
    except UnavailableVideoError:
        return await utils.answer(m, self.strings["noformat"])
    except XAttrMetadataError as XAME:
        return await utils.answer(m, self.strings["xameerr"].format(XAME))
    except ExtractorError:
        return await utils.answer(m, self.strings["exporterr"])
