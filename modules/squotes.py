...
class ShitQuotesMod:
    ...
    strings = {
        "name": "SQuotes",
        "no_reply": "**[SQuotes]**Нет реплая",
        "processing": "**[SQuotes]** Обработка...",
        "api_processing": "**[SQuotes]** Ожидание API...",
        "api_error": "**[SQuotes]** Ошибка API",
        "loading_media": "**[SQuotes]** Отправка...",
        "no_args_or_reply": "**[SQuotes]** Нет аргументов или реплая",
        "args_error": "**[SQuotes]** При обработке аргументов произошла ошибка. Запрос был: `{}`",
        "too_many_messages": "**[SQuotes]** Слишком много сообщений. Максимум: `{}`",
    }

    def __init__(self, client: telethon.TelegramClient, db: dict):
        self.client = client
        self.db = db
    ...
    async def sqsetcmd(self, message: Message):
        ...
        if not args:
            return await utils.answer(
                ...
                f"Настроить можно с помощью `.sqset` <параметр> <значение> или `reset`",
            )

        if args[0] == "reset":
            self.get_settings(True)
            return await utils.answer(
                message, "**[SQuotes - Settings]** Настойки квот были сброшены"
            )

        if len(args) < 2:
            return await utils.answer(
                message, "**[SQuotes - Settings]** Недостаточно аргументов"
            )
        ...
        if args[0] not in mods:
            return await utils.answer(
                message,
                f"**[SQuotes - Settings]** Такого парамерта нет, есть {', '.join(mods)}",
            )
            ...
            if args[1].lower() not in ["true", "false"]:
                return await utils.answer(
                    message,
                    "**[SQuotes - Settings]** Такого значения параметра нет, есть true/false",
                )
            ...
            if not args[1].isdigit():
                return await utils.answer(
                    message, "**[SQuotes - Settings]** Это не число"
                )
        ...
        return await utils.answer(
            message,
            f"**[SQuotes - Settings]** Значение параметра {args[0]} было выставлено на {args[1]}",
        )

    def get_settings(self, force: bool = False):
        settings: dict = self.db.get("SQuotes", {})
